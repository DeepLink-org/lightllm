...
def _token_softmax_reducev(logics, v, out, req_to_tokens, b_req_idx, b_start_loc, b_seq_len, max_input_len, other_kv_index):
    ...
    q_head_num = out.shape[1]
    batch, v_head, dim = b_loc.shape[0], v.shape[1], v.shape[2]
    for i in range(batch):
        ...
        with record_function('opt_step2'):
            P = opt_step2(P)
        P = P.reshape(q_head_num, 1, 1, b_seq_len[i]).transpose(0, 1)
        V = v[v_loc, :].view(1, b_seq_len[i], v_head, dim).transpose(1, 2).clone()
        with record_function('opt_step3'):
            res = opt_step3(P, V)
        out[i, :] = res.view(1, q_head_num, dim)
        return out
