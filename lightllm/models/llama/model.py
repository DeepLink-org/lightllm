...
class LlamaTpPartModel(TpPartBaseModel):
    ...
    def _init_to_get_dynamic_ntk_rotary(self):
        ...
        self._cos_cached = torch.zeros((max_seq_len, self.head_dim_ // 2), dtype=torch.float16, device="cpu")
        self._sin_cached = torch.zeros((max_seq_len, self.head_dim_ // 2), dtype=torch.float16, device="cpu")
        
        inv_freq = 1.0 / (base ** (torch.arange(0, self.head_dim_, 2, device="cpu", dtype=torch.float32) / self.head_dim_))
        t = torch.arange(max_position_embeddings, device="cpu", dtype=torch.float32)
        freqs = torch.outer(t, inv_freq)
        self._cos_cached[0:max_position_embeddings, :] = torch.cos(freqs).to(torch.float16)
        self._sin_cached[0:max_position_embeddings, :] = torch.sin(freqs).to(torch.float16)
        ...
        for seq_loc_index in range(max_position_embeddings, max_seq_len, 1):
            ...
            freqs = torch.outer(t, inv_freq)
            self._cos_cached[seq_loc_index:seq_loc_index + 1, :] = torch.cos(freqs).to(torch.float16)
            self._sin_cached[seq_loc_index:seq_loc_index + 1, :] = torch.sin(freqs).to(torch.float16)
        return
    ...
    def _init_to_get_yarn_rotary(self):
        ...
        inv_freq_mask = (1 - linear_ramp_mask(low, high, dim // 2).float()) * extrapolation_factor # Get n-d rotational scaling corrected for extrapolation
        ...
        t = torch.arange(max_seq_len_cached, device="cpu", dtype=torch.float32)
        freqs = torch.einsum("i,j->ij", t, inv_freq)
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = torch.cat((freqs, freqs), dim=-1)
        self._cos_cached = emb.cos().to(torch.float16) * mscale
        self._sin_cached = emb.sin().to(torch.float16) * mscale
