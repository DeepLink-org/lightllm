...
def apply_penalty_torch(Logits, presence_penalty, freqency_penalty, repetition_penalty, p_token_ids, 
                        p_token_counts, p_cumsum_seq_len, p_max_len_in_batch):
    cur_batch_start_index = p_cumsum_seq_len[0]
...
def apply(Logits, slice, presence_penalty, freqency_penalty, repetition_penalty, p_token_ids, p_token_counts):
    ...
    rep_logits = torch.where(cur_logits > 0, cur_logits / repetition_penalty[0], cur_logits * repetition_penalty[0])

    rep_logits = rep_logits - cur_token_counts * freqency_penalty[0] - presence_penalty[0]
    Logits[0, cur_token_ids] = rep_logits
