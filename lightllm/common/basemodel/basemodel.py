...
class TpPartBaseModel:
    ...
    def __init__(self, kvargs):
        ...
        self.opt_all_layer_token_forward = torch.compile(self.all_layer_token_forward, backend='ascendgraph', dynamic=False)
        return
    ...
    def get_batch_kv_index(self, Req_to_tokens, B_req_idx, b_seq_len, max_input_len, batch):
        ...
        for i in range(batch):
            # index = max_input_len - b_seq_len[i] + torch.arange(0, b_seq_len[i], device=b_seq_len.device)
    ...
    def _token_forward(self, input_ids, infer_state: InferStateInfo):
        ...
        with record_function('token_forward'):
            ...
            input_embs = self.opt_all_layer_token_forward(input_embs, infer_state, kv_index)

        with record_function('pos_process'):
            predict_logics = self.post_infer.token_forward(input_embs, infer_state, self.pre_post_weight, return_logics=True)
