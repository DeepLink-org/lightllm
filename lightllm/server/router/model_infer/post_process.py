...
def _top_p_top_k(probs: torch.Tensor, top_ps: torch.Tensor, top_ks: torch.Tensor):    
    ...
    probs_sort[torch.arange(0, probs.shape[-1], device="npu").view(1, -1) >= top_ks.view(-1, 1)] = 0.0
...
def _get_post_sample_tensors(reqs):
    ...
    presence_penalties = torch.tensor(presence_penalties, dtype=torch.float, device="npu")
    frequency_penalties = torch.tensor(frequency_penalties, dtype=torch.float, device="npu")
    repetition_penalties = torch.tensor(repetition_penalties, dtype=torch.float, device="npu")
    temperatures = torch.tensor(temperatures, dtype=torch.float, device="npu")
    top_ps = torch.tensor(top_ps, dtype=torch.float, device="npu")
    top_ks = torch.tensor(top_ks, dtype=torch.int32, device="npu")
    p_token_ids = torch.tensor(p_token_ids, dtype=torch.int32, device="npu")
    p_token_counts = torch.tensor(p_token_counts, dtype=torch.int32, device="npu")
    p_seq_len = torch.tensor(p_seq_len, dtype=torch.int32, device="npu")
