...
import logging
import argparse
from utils import Scheduler,_print_to_streams
...
class OutlineScheduler(Scheduler):
    ...
    def print_info(self):
        super().print_info()
        logging.info(
            "OutlineScheduler *outline prompt*: ", "magenta"
            + f"'''{self._outline_prompt}'''"
        )
        logging.info(
            "OutlineScheduler *point prompt*: ", "magenta"
    ...
    def stream_output(self, output_generator, streams):
        ...
        output_text = "" 
        cur_stage = "outline"
        logging.info("Outline scheduler outline:", "magenta")
        for outputs in output_generator:
            ...
            if not outputs["stage"] == cur_stage:
                ...
                _print_to_streams(streams, " ".join(output_text[pre:]), flush=True)
                _print_to_streams(streams, "\n\n", flush=True)
                logging.info("Outline scheduler response:", "magenta")
            ...
            if "sub_request" in outputs:
                ...
                logging.debug(
                    f"Sub-request {cur_point}: '''{sub_request}'''", "magenta"
                )
    ...
    def _get_response_stream(self, request, history=""):
        ...
        logging.debug(f"Outline request: {outline_request}\n----", "magenta")
